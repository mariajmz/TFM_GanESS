...
###  dependency.
###
### The GanESS Collaboration
### ------------------------------------------------------------------

import os
import subprocess

## Geant4 version required by simGaness
MIN_G4VERSION_NUMBER = 1100

## TFM_GanESS source code directories
SRCDIR = ['actions',
          'generators',
          'geometries',
          'materials',
...
def AssertG4Version(path):
    ...
    if version == b'':
        ...
        if g4version < MIN_G4VERSION_NUMBER:
            msg = 'This version of simGaness requires Geant4 version >= ' \
...
nexus = env.Program('bin/ganess', ['source/ganess.cc']+src)
...
nexus_test = env.Program('bin/ganess-test', ['source/ganess-test.cc']+tst+src)
